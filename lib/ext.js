...
function getDetailLinks($) {
  ...
  $('a[href]').each((_, element) => {
    ...
    const isExtTo = href.startsWith(EXT_BASE_URL);
    ...
    links.add(isExtTo ? href.replace(EXT_BASE_URL, '') : href);
  });

  return Array.from(links);
}

async function fetchDetailMagnets(detailUrl) {
  ...
  const proxyUrl = `https://r.jina.ai/http://ext.to${detailUrl.replace(
    EXT_BASE_URL,
    ''
  )}`;
...
async function fetchExtResults(imdbId, options = {}) {
  ...
  const scrapeDetailStreams = async (detailLinks) => {
    const streams = [];
    const seen = new Set();
    ...
    for (const path of detailLinks) {
      const detailUrl = path.startsWith('http')
        ? path
        : `${EXT_BASE_URL}${path}`;
      const magnets = await fetchDetailMagnets(detailUrl);
  ...
  if (response.ok) {
    ...
    const detailLinks = getDetailLinks($).slice(0, maxDetails);
    if (detailLinks.length) {
      const detailStreams = await scrapeDetailStreams(detailLinks);
  ...
  const proxyHtml = await proxyResponse.text();
  const { streams, $ } = extractStreams(proxyHtml);
  if (streams.length > 0) {
    return streams;
  }
