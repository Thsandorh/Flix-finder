...
async function webtor(magnet) {
  ...
  try {
    // 1. Regisztráljuk a torrentet az API-nál
    await request('https://webtor.io/api/v1/torrent', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ magnet })
    }).catch(() => null);

    // 2. Lekérjük a fájlokat a fő API-ból
    const info = await request(`https://webtor.io/api/v1/torrent/${hash}`, {}).catch(() => null);
    ...
    if (info?.files?.length) {
      ...
      // 3. Megpróbálunk tokent szerezni
      ...
      // 4. Elindítjuk a "renderelést" (mint a gomb az oldalon)
      await request(`https://webtor.io/api/v1/torrent/${hash}/exporter/mp4/render?fileId=${fileId}`, {}).catch(() => null);

      if (token) {
        // Közvetlen letöltési link a tokennel
