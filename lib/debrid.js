...
async function webtor(magnet) {
  ...
  try {
    // 1. Trigger the download job to get a Job ID
    // The website calls this when you click "Download Directory as ZIP"
    const enqueueUrl = `https://webtor.io/queue/download/enqueue/${hash}`;
    const enqueueRes = await request(enqueueUrl, {
      method: 'GET',
      headers: { 'Accept': 'application/json' }
    }).catch(() => null);

    const jobId = enqueueRes?.jobId || enqueueRes?.id || hash;

    // 2. Fetch the Job log using the Job ID
    const logUrl = `https://webtor.io/queue/download/job/${jobId}/log`;
    const logData = await request(logUrl, {}).catch(() => null);

    if (logData && typeof logData === 'string') {
      // Extract the direct link from the script block in the log
      const urlMatch = logData.match(/var\s+url\s+=\s+\"([^\"]+)\"/);
      if (urlMatch && urlMatch[1]) {
        let finalUrl = urlMatch[1].replace(/\\u0026/g, '&');
        
        // If it's a ZIP, we try to convert it to a streamable link by removing the suffix
        // but often the ZIP link itself works if the player supports it.
        return { 
          url: finalUrl, 
          title: '🟢 Webtor [Direct Link]', 
          name: 'WT' 
        };
      }
    }
  ...
  return { 
    ...
    title: '🟡 Open Webtor to start stream', 
    name: 'WT' 
  };
}
