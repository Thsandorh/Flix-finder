...
async function webtor(magnet) {
  ...
  // Webtor website flow simulation
  const websiteUrl = `https://webtor.io/${hash}`;
  const base = 'https://api.webtor.io/v1/torrent';
  
  let info;
  try {
    // Try to get info from API first
    info = await request(`${base}/${hash}`, {}).catch(() => null);
    
    if (!info) {
      // Try to add it via POST
      await request(base, {
        method: 'POST',
    ...
    // API failed, we'll use fallback below
  }

  if (info?.files?.length) {
    const fileSize = file => file?.size || file?.length || file?.bytes || 0;
    const file = info.files.reduce((a, b) => (fileSize(a) > fileSize(b) ? a : b));
    const fileId = file?.id ?? file?.index ?? 0;
    
    // This URL pattern often works or redirects to the direct cosmic-crab link
    const url = `https://webtor.io/api/v1/torrent/${hash}/file/${fileId}/download`;
    return { url, title: file.name || info.name, name: 'WT' };
  }

  // Final fallback: return the webtor page URL
  // Some Stremio clients can handle this or the user can open it manually
  return { 
    url: websiteUrl, 
    title: 'Stream on Webtor.io', 
    name: 'WT' 
  };
}
