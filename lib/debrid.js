...
async function webtor(magnet) {
  ...
  try {
    // Megpróbáljuk több forrásból is lekérni az adatokat
    let info = await request(`https://api.webtor.io/v1/torrent/${hash}?api-key=${apiKey}`, {}).catch(() => null);
    
    if (!info) {
      // Ha nincs meg, megpróbáljuk "beküldeni"
    ...
    if (info?.files?.length) {
      ...
      if (token) {
        // Közvetlen link összeállítása (cosmic-crab minta)
        const fileName = encodeURIComponent(file.name);
        // A 'abra' az egyik szerverük, megpróbáljuk a direkt mintát
        const url = `https://abra.api.cosmic-crab.buzz/${hash}/${fileName}?api-key=${apiKey}&token=${token}`;
        return { url, title: file.name, name: 'WT' };
      }

      // Fallback a Webtor belső download linkjére
      const downloadUrl = `https://webtor.io/api/v1/torrent/${hash}/file/${fileId}/download`;
      return { url: downloadUrl, title: file.name, name: 'WT' };
    }
  } catch (e) {
    // Hiba esetén fallback
  }

  // Ha nem sikerült adatot kinyerni
  return { 
    url: websiteUrl, 
    title: '🟢 Open in Webtor (API fail)', 
    name: 'WT' 
  };
}
